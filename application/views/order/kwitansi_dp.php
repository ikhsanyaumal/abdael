<?php 
	...
	{	
		function print_kuitansi($project,$order,$payment,$history_transaksi){
			...
			$this->Cell(60,5,$history_transaksi['name'],0,1,'L');
			...
			$terbilang = ucwords(Terbilang($history_transaksi['kwitansi_payment']));
			...
			$this->Cell(3,5,' : ',0,0,'C');
			if ($history_transaksi['period']=='0') {
				$this->Cell(55,5,'Uang Tanda Jadi (UTJ) pembelian Rumah / Ruko ',0,1,'L');
			}else{
				$this->Cell(55,5,'Uang muka ke '.$history_transaksi['period'].' pembelian Rumah / Ruko ',0,1,'L');
			}
			...
			$kwitansi_date = date_create($history_transaksi['date']);
			$this->ln();
			$this->Cell(140,5,'',0,0,'L');
			...
			// }elseif ($history_transaksi['period']=='0') {
			// 	$this->SetFont('times','I',8);
			// 	$this->Cell(140,5,'* bila pada waktu jatuh tempo DP 1 tidak ada pembayaran, maka transaksi ini dianggap batal dan uang ikatan menjadi hangus',0,1,'L');
			}

			$tanda_jadi = number_format($history_transaksi['kwitansi_payment'], 0, ',', '.');
			$this->SetXY($x,$y+10);
		...
		function print_kuitansi_copy($project,$order,$payment,$history_transaksi){
	...
	$pdf->print_kuitansi($project,$order,$payment,$history_transaksi);
